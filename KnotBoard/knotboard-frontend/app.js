  ...
  return `
    <div class="shell">
      <header class="topbar">
        ...
        <div class="user-chip">
          <span class="role-pill">${escapeHtml(state.auth.username)} · ${escapeHtml(state.auth.role)}</span>
        ...
        <aside class="sidebar">
          ...
          <section class="form-panel">
            <div class="panel-title">
              ...
              ${state.editingBoard ? `<button class="icon-button" title="Cancel edit" data-action="cancel-board-edit">×</button>` : ""}
            </div>
            ...
            <div class="board-list">
              ${boards.map((item) => `
                <button class="board-button ${item.id === board?.id ? "active" : ""}" data-board-id="${item.id}">
                  ...
                  <span>${escapeHtml(item.createdByUsername || "Unknown")} · ${formatDate(item.createdAt)}</span>
                </button>
            ...
            <section class="panel">
              <div class="board-header">
                <div>
                  ...
                  <p>Created by ${escapeHtml(board.createdByUsername || "Unknown")} · ${formatDate(board.createdAt)}</p>
                </div>
...
function renderNote(note) {
  return `
    <article class="note-card">
      ...
      <div class="note-meta">
        <span>${escapeHtml(note.createdByUsername || "Unknown")} · ${formatDate(note.createdAt)}</span>
        <span class="note-actions">
          <button class="icon-button" title="Edit note" data-action="edit-note" data-note-id="${note.id}">✎</button>
          <button class="icon-button" title="Delete note" data-action="delete-note" data-note-id="${note.id}">×</button>
        </span>
      </div>
    </article>
  `;
